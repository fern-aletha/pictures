...
export function detectPlatform(customUserAgent?: NavigatorID['userAgent'], customUserAgentData?: NavigatorUAData): Platform {
  if (!isClient) {
    return {
      os: empty,
      type: empty,
  ...
  if (userAgent) {
    ...
    const browser = (browserResult && browserResult[0]) || empty;
    const version = (saVesion ? saVesion : browserResult && browserResult[1]) || empty;
